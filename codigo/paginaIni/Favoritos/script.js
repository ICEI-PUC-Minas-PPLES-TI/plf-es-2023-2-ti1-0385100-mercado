...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const apiUrlAtividades =
    "https://jsonserver-tiaw.igorfernandesca.repl.co/produtos";
  const apiUrlFavoritos =
    "https://jsonserver-tiaw.igorfernandesca.repl.co/favoritos";
      ...
      .then((data) => {
        ...
        data.forEach((atividade) => {
          const isFavorito = favoritos.some(
            (favorito) => favorito.id === atividade.id
          );
  ...
  function criarCard(atividade, isFavorito, isInterativo) {
    ...
    <div class="img-container">
      <img src="${atividade.imagem}" alt="" class="image card-img-top">
    </div>    
      <h3>${atividade.nome}</h3>
        <p>Mercado: ${atividade.mercado}</p>
        <p>Preço: R$ ${atividade.preço}</p>
        <p>Categoria: ${atividade.categoria}</p>
        <p>Descrição: ${atividade.descriçao}</p>
      <i id="favorito-${atividade.id}" class="far fa-heart favorito ${
      isFavorito ? "fas" : ""
    }"></i>
    `;
  ...
  function removerFavorito(atividade) {
    fetch(`${apiUrlFavoritos}/${atividade.id}`, {
      ...
      const cardNaLista = document.querySelector(
        `#favoritoList-${atividade.id}`
      );
      if (cardNaLista) {
        cardNaLista.remove();
      }

      const cardNaTela2 = document.querySelector(
        `#favoritoScreen-${atividade.id}`
      );
      if (cardNaTela2) {
        cardNaTela2.remove();
      }
